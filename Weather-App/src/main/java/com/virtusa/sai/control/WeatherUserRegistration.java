...
//WeatherUserRegistration is a Controller
@Controller
public class WeatherUserRegistration {
	private static final Logger LOGGER = LoggerFactory.getLogger(WeatherUserRegistration.class); // Logging for WeatherUserRegistration class.
	@Autowired
	...
	@GetMapping("/weatherreg")//URL pattern for getting values from Registration page
	...
	@PostMapping("/weatherreg")//URL pattern for inserting the new user details in database.
	public String PostRegister(@ModelAttribute("user") UserRegistration user,Model model,HttpServletRequest request) {
		...
		try {
			// Stored Procedure to insert a new user details in Database.
